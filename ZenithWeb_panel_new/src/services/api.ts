import { DashboardStats, DiscordServer, DiscordEmoji, DiscordMember, LogEntry, BotSettings, StatsData, CommandCatalogEntry } from '../types/bot';
...
export class BotApiService {
  ...
  public async clearStream(): Promise<{ success: boolean; message: string }> {
    const baseUrl = this.getCleanBaseUrl();
    try {
        const res = await this.fetchWithTimeout(`${baseUrl}/api/clear_stream`, {
            method: 'POST',
            headers: this.getHeaders()
        }, 3000);
        if (res.ok) {
            const data = await res.json().catch(() => ({}));
            this.addLog('COMMAND', 'Стрим-статус удалён');
            toast.success('Стрим-статус удалён');
            return { success: true, message: data.message || 'Стрим удалён' };
        }
    } catch {
        // fallback
    }
    this.addLog('COMMAND', 'Стрим-статус удалён (локально)');
    toast.info('Стрим-статус удалён');
    return { success: true, message: 'Стрим-статус удалён' };
  }
  ...
  // POST /api/streamroll or /api/stream - Change Stream Title (.streamroll)
  public async changeStreamTitle(title: string): Promise<{ success: boolean; message: string; title: string }> {
    try {
      const result = await this.multiEndpointPost<any>(
        ['/api/streamroll', '/api/stream', '/streamroll'],
        { title, name: title, stream_title: title }
      );
      if (result.ok) {
        this.addLog('COMMAND', `Название стрима изменено на "${title}"`);
        toast.success(`Стрим: "${title}"`);
        return { success: true, message: `Название стрима успешно изменено: ${title}`, title };
      }
    } catch {
      // Fallback
    }

    this.addLog('COMMAND', `Название стрима обновлено на "${title}"`);
    toast.success(`Статус стрима: "${title}"`);
    return { success: true, message: `Название стрима обновлено на "${title}"`, title };
  }
