...
const MainAppContent: React.FC = () => {
  ...
  // Sync theme class to document body and root
  useEffect(() => {
  ...
  const handleToggleTheme = () => {
    ...
    soundFX.playClick();
    showToast(nextTheme === 'light' ? 'Включена светлая контрастная тема' : 'Включена тёмная киберпанк тема', 'info');
  };
  ...
  const handleSetGameStatus = async (gameName: string) => {
    const res = await botApi.setGameStatus(gameName);
    const lgs = await botApi.getLogs();
    setLogs(Array.isArray(lgs) ? lgs : []);
    return res;
  };
        ...
        <main className="flex-1 p-6 md:p-8 max-w-7xl w-full mx-auto">
          ...
          {currentTab === 'commands' && (
            <CommandsTab
              ...
              onSetGameStatus={handleSetGameStatus}
