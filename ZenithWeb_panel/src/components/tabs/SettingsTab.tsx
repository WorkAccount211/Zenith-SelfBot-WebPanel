        ...
        <div className="p-6 rounded-2xl bg-[#131024]/70 border border-purple-500/20 backdrop-blur-xl shadow-lg space-y-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="font-bold text-white text-base">Тема оформления интерфейса</h3>
                <p className="text-xs text-purple-300/60">
                  Выбор между классической тёмной киберпанк темой и высококонтрастной светлой темой
                </p>
              </div>
            </div>
            <span className="text-xs font-mono px-2.5 py-1 rounded-lg bg-purple-500/10 border border-purple-500/20 text-purple-300">
              {settings.theme === 'light' ? 'Светлая (Light Mode)' : 'Тёмная (Dark Mode)'}
            </span>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2">
            <button
              ...
              onClick={() => {
                ...
                soundFX.playClick();
                showToast('Включена тёмная киберпанк тема', 'info');
              }}
              className={`p-4 rounded-xl border transition-all flex items-center gap-3.5 text-left ${
                settings.theme !== 'light'
                  ? 'bg-purple-950/50 border-purple-400 shadow-[0_0_18px_rgba(168,85,247,0.25)] ring-1 ring-purple-400/50'
                  : 'bg-[#090812] border-purple-500/20 hover:border-purple-500/40 text-purple-300/70 hover:text-white'
              }`}
            >
              <div className="p-3 rounded-xl bg-[#090812] border border-purple-500/30 text-purple-400 shrink-0">
                <Moon className="w-6 h-6 text-purple-300" />
              </div>
              <div className="min-w-0">
                <div className="flex items-center gap-2">
                  <span className="font-bold text-sm text-white">Тёмная кибер-тема</span>
                  {settings.theme !== 'light' && (
                    <span className="text-[10px] bg-purple-500/30 text-purple-200 px-1.5 py-0.5 rounded font-mono font-bold">
                      АКТИВНА
                    </span>
                  )}
                </div>
                <p className="text-xs text-purple-300/60 mt-0.5">
                  Глубокий неоновый фон (#090812), фиолетовые акценты и ночной режим
                </p>
              </div>
            </button>
            ...
            <button
              ...
              onClick={() => {
                ...
                soundFX.playClick();
                showToast('Включена светлая высококонтрастная тема', 'info');
              }}
              className={`p-4 rounded-xl border transition-all flex items-center gap-3.5 text-left ${
                settings.theme === 'light'
                  ? 'bg-purple-900/20 border-purple-500 shadow-[0_0_18px_rgba(168,85,247,0.25)] ring-1 ring-purple-500'
                  : 'bg-[#090812] border-purple-500/20 hover:border-purple-500/40 text-purple-300/70 hover:text-white'
              }`}
            >
              ...
              <div className="min-w-0">
                <div className="flex items-center gap-2">
                  <span className="font-bold text-sm text-white">Светлая контрастная тема</span>
                  {settings.theme === 'light' && (
                    <span className="text-[10px] bg-amber-500/30 text-amber-900 dark:text-amber-200 px-1.5 py-0.5 rounded font-mono font-bold">
                      АКТИВНА
                    </span>
                  )}
                </div>
                <p className="text-xs text-purple-300/60 mt-0.5">
                  Белый чистый фон, высокая контрастность для дневной работы
                </p>
              </div>
            </button>
          </div>
